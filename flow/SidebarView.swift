            ...
            VStack(spacing: 8) {
                ...
                HStack {
                    ...
                    HStack(spacing: 10) {
                        ...
                        Button(action: { appState.openRightPanel(.extensions) }) {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        if (!pinnedTabs.isEmpty || !pinnedFolders.isEmpty)
                            && (!unpinnedFolders.isEmpty || !ungroupedTabs.isEmpty)
                        {
                            Divider()
                                .padding(.vertical, 6)
                                .opacity(0.6)
            ...
            Menu("Add to Folder") {
                ForEach(store.folders) { folder in
                    Button(action: { store.assign(tabID: tab.id, toFolder: folder.id) }) {
                        Label(
                            folder.name,
                            systemImage: (tab.folderID == folder.id) ? "checkmark" : "folder")
                    }
                }
                Divider()
                Button("New Folder…") {
    ...
    private var faviconView: some View {
        ...
        private func createFolderPromptAndAssign() {
            ...
            if response == .alertFirstButtonReturn {
                ...
                let palette = [
                    "#5E81AC", "#A3BE8C", "#EBCB8B", "#D08770", "#BF616A", "#B48EAD", "#88C0D0",
                ]
